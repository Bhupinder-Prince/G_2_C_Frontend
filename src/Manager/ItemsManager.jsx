      ...
      <div className="container">
        ...
        <div className="border-b border-gray-900/10 pb-4">
              <img
                  ...
                  alt="Your Company"
                />
              <h2 className="text-base text-lg font-semibold leading-7 text-gray-900">Manage the Items</h2>
              
            </div>
          <div className="grid gap-6 mb-6 md:grid-cols-2">
            <div>
                <p className="flex flex-wrap">
                <label htmlFor="emailid" className="block mb-2 mt-4 text-base font-serif font-medium text-gray-900">Email Address&nbsp;&nbsp;</label>
                </p>
                <input type="email" id="email" name="email" onChange={doUpdate} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-100 block w-full p-2.5 dark:bg-gray-100 dark:border-gray-100 dark:text-black dark:focus:ring-blue-300 dark:focus:border-blue-500" required/>
            </div>
            <div>
              <button type="button" onClick={doFetchProduct} className=" block mt-16 ml-[23px] w-[100px] rounded-md bg-green-600 px-3.5 py-2.5 text-center text-sm font-semibold text-white shadow-sm hover:bg-green-500 focus-visible:outline focus-visible:outline-2  focus-visible:outline-green-600">
                  Search
              </button> 
            </div>
          </div>
              ...
              {result.map((obj,index)=>{             
                return (
                  <tr key={index}>    
                    ...
                    <td align="center">
                      <button onClick={()=>{doDeleteProduct(obj.items)}} class="btn btn-outline-danger text-dark">Delete</button>
                    </td>
                  </tr>
              )})} 
            </table>
